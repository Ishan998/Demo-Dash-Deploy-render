...
const ShopByOccasion: React.FC<Props> = ({ items, onSelect }) => {
  ...
  const handleImageLoad = (id: number, e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    // Taller images get more rows; clamp to keep layout tidy.
    ...
    <section className="py-20 bg-[#FDFBF6]">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-gray-800">Shop by Occasion</h2>
          <p className="text-gray-600 mt-2">Find the perfect piece for every milestone and memory.</p>
        </div>
        <div
          ref={gridRef}
        ...
        >
          {data.map((occasion) => (
            <a
              href="#"
              key={occasion.id}
              className={`grid-item group relative overflow-hidden flex items-center justify-center rounded-xl shadow-sm ${occasion.gridClass || ''}`}
              style={{ gridRowEnd: `span ${rowSpans[occasion.id] || 1}` }}
              onClick={(e) => handleClick(e, occasion.name)}
            >
              <img
                ...
                loading="lazy"
                onLoad={(e) => handleImageLoad(occasion.id, e)}
              />
              <div className="absolute inset-0 bg-black/40 transition-colors group-hover:bg-black/50"></div>
              <div className="relative text-center text-white p-4">
                <h3 className="text-3xl font-semibold tracking-wider">{occasion.name}</h3>
                <div className="mt-4 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                    <span className="py-2 px-6 border border-white rounded-full">Explore Now</span>
                </div>
              </div>
            </a>
          ))}
